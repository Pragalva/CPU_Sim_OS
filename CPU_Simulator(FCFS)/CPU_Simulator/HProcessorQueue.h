...
public:
	...
	void update(int t)
	{
		if (treating->end_run_time != -1)
		{
			...
			if ((treating->created + treating->wait_time + 2) % 17 == 0)
			{
		...
		if (lowP_Programs.empty())
		{
			...
			*p = HProcessor1->lowP_Programs.front();
			HProcessor1->lowP_Programs.pop();
			...
			if (report_output)
			{
				std::cout << "HProcessor starting run for " << p->program_name << " with priority " << p->priority_number << " at " << t << " it also had a I/O wait of " << IO_wait << std::endl;
			}
			*treating = *p;
		}
